...
fmd("alias",["config","event"],function(f,g){f.register({key:"alias",name:"object"});g.on("alias",function(e){var c=f.get("alias"),d;c&&(d=c[e.id])&&(e.nominalId=e.id,e.id=d)})});fmd("resolve",["event","config"],function(f,g){g.register({key:"resolve",name:"array"});f.on("resolve",function(e){var c=g.get("resolve"),d;if(c)for(var a=0,b=c.length;a<b&&(d=c[a](e.id),void 0===d||d===e.id);a++);e.uri=d?d:e.id})});
fmd("id2url",["global","event","config"],function(f,g,e){var c=/^https?:\/\//i,d=(new Date).getTime();e.set({baseUrl:function(){var a=/^\w+\:\/\/[\w\-\.:]+\//i,b=f.document.getElementsByTagName("script"),b=b[b.length-1];return(a=(b=b.hasAttribute?b.src:b.getAttribute("src",4))?b.match(a):null)?a[0]:""}()});e.register({key:"stamp",name:"object"});g.on("resolve",function(a){a.url=a.uri});g.on("stamp",function(a){var b=e.get("hasStamp")?d:null,c=e.get("stamp");if(c)for(var g in c)if((new RegExp(g)).test(a.id)){b=
c[g];break}b&&(a.url+="?fmd.stamp="+b)});g.on("id2url",function(a){g.emit("resolve",a);c.test(a.url)||(a.url=e.get("baseUrl")+a.url);var b=a.url;b.lastIndexOf(".")<b.lastIndexOf("/")&&(a.url+=".js");g.emit("stamp",a)});g.on("id2uri",function(a){g.emit("id2url",a);a.uri=a.url})});
fmd("assets",["cache","lang","event","config","module"],function(f,g,e,c,d){var a=f.assets={},b={},l={make:function(c,g){var f={id:c};e.emit("analyze",f);e.emit("alias",f,g);if(b[f.id])return a[b[f.id]];d.has(f.id)?f.uri=f.id:e.emit("id2uri",f);b[f.id]=f.uri;return a[f.uri]=f},group:function(a){return g.map(a.deps,function(b){return l.make(b,a)})}};return l});
