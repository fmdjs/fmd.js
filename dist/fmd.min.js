...
a;d.emit("saved",a);a.autocompile()};h.require=function(a){var b=h.get(a);if(!b)return d.emit("requireFailed",{id:a}),null;b.compiled||(b.compiled=!0,b.compile());d.emit("required",b);return b.exports};h.makeRequire=function(a){a.require=function(b){b={id:b};d.emit("relative",b,a);d.emit("alias",b);return h.require(b.id)}};h.define=function(b,k,e){var f=arguments.length;1===f?(e=b,b=""):2===f&&(e=k,k=a,c.isString(b)||(k=b,b=""));if(h.has(b,!0))return d.emit("existed",{id:b}),null;h.save(new h(b,k,
e))};h.compile=function(a){if(c.isFunction(a.factory)){var b=a.extract(),b=a.factory.apply(null,b);void 0!==b?a.exports=b:a.module&&a.module.exports&&(a.exports=a.module.exports);a.module&&delete a.module}else void 0!==a.factory&&(a.exports=a.factory);d.emit("compiled",a)};h.define.fmd={};var r=g.define;f.noConflict=function(){g.define=r};f.define=g.define=h.define;return h});
...
fmd("id2url",["global","event","config"],function(g,f,e){var c=/^https?:\/\//i,d=(new Date).getTime();e.set({baseUrl:function(){var b=/^\w+\:\/\/[\w\-\.:]+\//i,a=g.document.getElementsByTagName("script"),a=a[a.length-1];return(b=(a=a.hasAttribute?a.src:a.getAttribute("src",4))?a.match(b):null)?b[0]:""}()});e.register({keys:"resolve",name:"array"}).register({keys:"stamp",name:"object"});f.on("resolve",function(b){var a=e.get("resolve"),d;if(a)for(var c=0,f=a.length;c<f&&(d=a[c](b.id),d===b.id);c++);
b.url=d?d:b.id});f.on("stamp",function(b){var a=e.get("hasStamp")?d:null,c=e.get("stamp");if(c)for(var f in c)if(RegExp(f).test(b.id)){a=c[f];break}a&&(b.url+="?fmd.stamp="+a)});f.on("id2url",function(b){f.emit("resolve",b);c.test(b.url)||(b.url=e.get("baseUrl")+b.url);var a=b.url;a.lastIndexOf(".")<a.lastIndexOf("/")&&(b.url+=".js");f.emit("stamp",b)})});
fmd("assets",["cache","lang","event","config","module"],function(g,f,e,c,d){var b=g.assets={},a={},k={make:function(c,f){var h={id:c};e.emit("analyze",h);e.emit("relative",h,f);e.emit("alias",h);if(a[h.id])return b[a[h.id]];d.has(h.id)?h.url=h.id:e.emit("id2url",h);a[h.id]=h.url;return b[h.url]=h},group:function(a){return f.map(a.deps,function(b){return k.make(b,a)})}};return k});
